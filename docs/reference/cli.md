...
3. After all scheduled steps complete, the command is executed directly using the execution context's subprocess executor.
4. The subprocess inherits the full accumulated `ExecutionContext`, so it gets all installed tool paths in `PATH` and all environment variables set by prior steps.
